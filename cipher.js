    ...
    if ((msg !== undefined && msg.length > 0) && (num !== undefined && !isNaN(num))) {
        ...
        for (let i = 0; i < lower.length; i++) {
            ...
            if (!alpLow.includes(lower[i])) {
                ...
                //find the index number of the letter from "lower" in "alpLow" and add the @num, then assign it to the "newIndex".
                newIndex = alpLow.indexOf(lower[i]) + num;

                //if "newIndex" is greater than the length of Alphabet array OR less than 0:
                if (newIndex >= alpLow.length || newIndex < 0) {
                    //assign the new value with the "newIndex" modulus the length of the Alphabet array, which is 26. 
                    newIndex %= alpLow.length;

                    //if "newIndex" is a negative number, subtract it from length of the Alphabet array and then assign it.
                    if (newIndex < 0) { 
